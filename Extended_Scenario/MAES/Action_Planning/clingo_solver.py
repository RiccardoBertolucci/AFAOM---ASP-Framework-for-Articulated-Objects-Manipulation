...
rootdir = local_path + '/ASP/clingo/data'

def solution_name_generator(solution_name):
	solution_name = solution_name.replace("p_","")
	#solution_name = solution_name.split("-","")
	solution_name = solution_name.replace(".asp","")
...
for subdir, dirs, files in os.walk(rootdir):	
	...
	for file in files:
		...
		sdata_name = str(file)
		data_path = rootdir + "/" + folder_name+"/"+str(file)
		...
		command = "./pyrunlim.py 2> "+ times_path+" -t300 \"./wrapper_clingo.py " + solution_name + " " + data_path + " " + folder_name + "\""

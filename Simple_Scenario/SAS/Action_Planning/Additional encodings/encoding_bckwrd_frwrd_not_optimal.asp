...
{changeAngleF(J1,J2,A,Ai,T) : joint(J1), joint(J2), J1 > J2, angles(A), hasAngle(J1,Ai,T), A <> Ai, isLinked(J1,J2);changeAngleD(J1,J2,A,Ai,T): joint(J1), joint(J2), J1 < J2, angles(A), hasAngle(J1,Ai,T), A <> Ai, isLinked(J1,J2)} <=1 :- time(T), T < X, maxspan(X), T > 0.
...
hasAngle(J1,A,T+1) :- hasAngle(J1,A,T), not changeAngleF(J1,_,_,_,T), not affectedF(J1,_,_,T), not changeAngleD(J1,_,_,_,T), not affectedD(J1,_,_,T), T <= X, maxspan(X).
